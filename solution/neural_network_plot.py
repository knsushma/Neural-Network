...
class NeuralNetwork:
    ...
    def train_model(self, epoch):
        ...
        for row_index, row in enumerate(self.std_dataset):
            ...
            if ((sigmoid_activation_value >= 0.5 and y == 1) or (sigmoid_activation_value < 0.5 and y == 0)):
                corrects += 1
            cross_entropy_error += (-1.0 * y * math.log(sigmoid_activation_value) - (1.0 - y) * math.log(
                1 - sigmoid_activation_value))
    ...
    def prediction_on_testdate(self, epoch, test_obj):
        ...
        for index, row in enumerate(test_obj.std_dataset):
            ...
            if ((sigmoid_activation_value >= 0.5 and label_list[index] == 1) or (
                    sigmoid_activation_value < 0.5 and label_list[index] == 0)):
                corrects += 1
    ...
    def plot_F1_score_curve(self, x1, y1, y2):
        fig, ax = plt.subplots()
        textstr = "Max Epoch: {0}\nLearning Rate: {1}\nHidden Units: {2}\nDataset: Magic".format(self.max_epoch, self.learning_rate, self.num_of_hidden_units)
...
if __name__ == '__main__':
    ...
    if (len(sys.argv) < 6):
        print(
            "Please pass 5 arguments. 1) Learning Rate 2) Hidden Units 3) Epochs 4) Training File Path, 5) Testing File path ")
        sys.exit(1)
    ...
    np.random.shuffle(train_neural.std_dataset)
    np.random.shuffle(test_neural.std_dataset)
